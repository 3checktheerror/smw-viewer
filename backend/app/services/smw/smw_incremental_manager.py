...
from backend.app.api.schemas.wallet_schemas import SMWIncrementalModel
...
class SMWIncrementalManager:
    ...
    @staticmethod
    def get_daily_incremental_wallets() -> List[SMWIncrementalModel]:
        mongo_client = MongoDBClient()
        current_bg_date = TimeUtils.get_cur_bg_date()
        ...
        # 4. 组合结果数据并转换为SMWIncrementalModel
        result_wallets = []
        for record in incremental_data:
            address = record.get('address')
            chain = record.get('chain')
            ...
            # 创建SMWIncrementalModel对象
            incremental_wallet = SMWIncrementalModel(
                address=address,
                chain=chain,
                store_date=current_bg_date,
                pnl=pnl,
            ...
            result_wallets.append(incremental_wallet)
